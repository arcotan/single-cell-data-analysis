---
title: Clustering and differential expression analysis of PBMC2 using Seurat
---
...
```{r}
pbmc <- NormalizeData(pbmc, normalization.method = "LogNormalize", scale.factor = 10000) # default params
```

Feature selection

```{r}
pbmc <- FindVariableFeatures(pbmc, selection.method = "vst", nfeatures = 1800) # nfeatures chosen based on elbow plot above
...
```{r}
pbmc <- FindNeighbors(pbmc, dims = 1:20) # chosen based on elbow plot above
pbmc <- FindClusters(pbmc, resolution = 0.98) # should be 16 for PBMC2
```
...
```{r}
pbmc.markers <- FindAllMarkers(pbmc, min.pct = 0.1, logfc.threshold = 0.25) # FIXME: how to choose params? default min.pct is 0.01, default logfc.threshold is 0.1
```
