...
PBMC1 <- addClusterization(PBMC1, clName = "merged",
...
```{r, echo=TRUE, eval=TRUE}
mergedClusters <- getClusterizationData(PBMC1, clName = "merged")[[1]]
...
```{r, echo=TRUE, eval=FALSE}
saveRDS(PBMC1, file = file.path(OUT_DEFAULT_DIR, paste0(dataset_name, ".cotan.RDS")))
```
...
barcodes_to_drop = getCells(PBMC1)[!getCells(PBMC1) %in% barcodes_to_keep]
PBMC1_celltypist <- dropGenesCells(PBMC1, cells = barcodes_to_drop)
...
repeat{
  ...
  c(mergedClusters, mergedCoexDF) %<-%
  mergeUniformCellsClusters(PBMC1_celltypist, clusters = splitClusters,
                            GDIThreshold = resolution,
                            cores = 13,
                            saveObj = TRUE, outDir = OUT_DEFAULT_DIR)
  PBMC1_celltypist <- addClusterization(PBMC1_celltypist, clName = "merged_celltypist",
                               clusters = mergedClusters,
                               coexDF = mergedCoexDF,
                               override = TRUE)
  mergedClusters <- getClusterizationData(PBMC1_celltypist, clName = "merged_celltypist")[[1]]
...
```{r, echo=TRUE, eval=FALSE}
coexDF <- DEAOnClusters(PBMC1_celltypist, clusters = final_clusters)
PBMC1_celltypist <- addClusterizationCoex(PBMC1_celltypist, clName = final_clusters_name,
                             coexDF = coexDF)
```

```{r, echo=TRUE, eval=FALSE}
saveRDS(PBMC1, file = file.path(OUT_CELLTYPIST_DIR, paste0(dataset_name, ".cotan.RDS")))
```
...
numCells <- getNumCells(PBMC1_celltypist)
pvals <- pValueFromDEA(coexDF, numCells)
...
barcodes_to_drop = getCells(PBMC1)[!getCells(PBMC1) %in% barcodes_to_keep]
PBMC1_protein <- dropGenesCells(PBMC1, cells = barcodes_to_drop)
...
repeat{
  ...
  cat(paste("Trying resolution ", resolution, sep=''))
  c(mergedClusters, mergedCoexDF) %<-%
  mergeUniformCellsClusters(PBMC1_protein, clusters = splitClusters,
                            GDIThreshold = resolution,
                            cores = 13,
                            saveObj = TRUE, outDir = OUT_DEFAULT_DIR)
  PBMC1_protein <- addClusterization(PBMC1_protein, clName = "merged_protein",
                               clusters = mergedClusters,
                               coexDF = mergedCoexDF,
                               override = TRUE)
  mergedClusters <- getClusterizationData(PBMC1_protein, clName = "merged_protein")[[1]]
  num_clusters = nrows(table(mergedClusters))
  cat(paste("Got ", num_clusters, ' clusters', sep=''))
...
```{r, echo=TRUE, eval=FALSE}
coexDF <- DEAOnClusters(PBMC1_protein, clusters = final_clusters)
PBMC1_protein <- addClusterizationCoex(PBMC1_protein, clName = final_clusters_name,
                             coexDF = coexDF)
```

```{r, echo=TRUE, eval=FALSE}
saveRDS(PBMC1, file = file.path(OUT_PROTEIN_DIR, paste0(dataset_name, ".cotan.RDS")))
```
...
numCells <- getNumCells(PBMC1_protein)
pvals <- pValueFromDEA(coexDF, numCells)
