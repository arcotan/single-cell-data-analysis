...
if (file.exists(file.path(outDir, '/', "fineClusters.RDS"))){
    ...
    fineClusters <- cellsUniformClustering(obj, GDIThreshold = 2.0, cores = 8,
...
if (file.exists(file.path(outDir, '/', "mergedClusters.RDS")) ||
    ...
    c(mergedClusters, coexDF, pValueDF) %<-%
      mergeUniformCellsClusters(obj, clusters=fineClusters, GDIThreshold = 1.4, cores = 8, outDir = outDir)
    print(Sys.time() - start)
...
write_COTAN_markers(OUT_RES_DIR, mergedClusters, coexDF, TOP_MARKER_NUM)
...
umap = plotUMAP(gdumap, colour_by="label", text_by="label", text_color="red")
ggsave(paste(OUT_RES_DIR, "/umap.jpg", sep=""), umap, width = 10, height = 10)
```