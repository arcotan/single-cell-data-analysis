...
```{r}
file_pos = paste('./COTAN/', dataset_name, '.cotan.RDS', sep = "")
```
...
clusterFile = paste(outDir, '/', dataset_name, '/', 'Seurat_obj_with_cotan_clusters.RDS', sep="")
```
...
if (file.exists(clusterFile)) {
    ...
    fineClusters <- readRDS(clusterFile)
} else {
...
```{r eval=FALSE, include=TRUE}
c(coexDF, pValueDF) %<-% DEAOnClusters(obj, clusters = fineClusters$pca)
