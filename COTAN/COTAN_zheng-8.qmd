...
outDir = paste('./COTAN/full_', dataset_name, sep="")
...
fineClusters <- cellsUniformClustering(obj, GDIThreshold = 1.4, cores = 7,
...
srat = CreateSeuratObject(counts = obj@raw, project = dataset_name, min.cells = 0, min.features = 0)

tl = read.csv(paste(IN_LABEL_DIR, 'labels.csv', sep=''), header=TRUE)

# reorder tl such that tl$cell is in the same order as colnames(data)
tl = tl[match(colnames(data), tl$cell),]

true_ids = tl$cluster.ids

gdsce = as.SingleCellExperiment(srat)
gdsce = applySCE(gdsce, logNormCounts)
gdumap = runUMAP(gdsce)
colLabels(gdumap) <- factor(fineClusters)
gdumap[['true']] <- factor(true_ids %% 6)
umap = plotUMAP(gdumap, shape_by="true", colour_by="label", text_by="label", text_color="red")
plot(umap)
ggsave(paste(outDir, '/', dataset_name, '.umap.pdf', sep = ""), umap, width = 10, height = 10)
```
