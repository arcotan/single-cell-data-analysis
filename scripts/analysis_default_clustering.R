...
collect_dataset_data = function(tool_tag_list, dataset_tag, compute_missing_scores = TRUE, filtered_datasets_dir_map = NULL) {
  ...
  if (!to_init) {
    ...
    true_ids = read.csv(paste(DATASET_TAG_TO_TRUE_LABEL_DIR[[dataset_tag]], LABEL_FILENAME, sep=""))
    ...
    if (compute_missing_scores) {
      ...
      for (i in 1:nrow(score_data)) {
        ...
        if (is.na(cur_info$silhouette) && !is.null(filtered_datasets_dir_map)) {
          ...
          # compute PCA for ge
          geSeurat <- CreateSeuratObject(counts = ge)
          geSeurat <- NormalizeData(geSeurat, normalization.method = "LogNormalize", scale.factor = 10000)
    ...
    if (!is.null(dataset_data$labels) || !is.null(dataset_data$scores) || !is.null(dataset_data$markers)) {
      if (write_aggregate) {
        if (!dir.exists(paste(AGGREGATE_RESULT_DIR, tag, sep=""))) {
          dir.create(paste(AGGREGATE_RESULT_DIR, tag, sep=""), recursive = TRUE)
        }
...
for (dataset in dataset_found) {
  # load GO mapping
  go_mapping = read.csv(paste(DATASET_TAG_TO_MAPPING_DIR[[dataset]], MAPPING_FILENAME, sep=""))
